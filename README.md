...
User Action
  -> ViewModel (collects Flow from UseCase)
    -> UseCase (delegates to Repository)
      -> Repository (channelFlow):
          1. Observes Room Flow -> emits to UI (starts immediately)
          2. launch: Fetches from API -> writes to Room
          (on any error, Resource.Error is sent but Room observation continues)
...
```
app/src/main/java/com/vladan/offlinefirstarch/
├── core/
...
### Parallel Data Loading with channelFlow

Both `SearchRepositoryImpl` and `DetailsRepositoryImpl` use `channelFlow` to observe Room and fetch from API concurrently. The Room observer starts collecting immediately while the network call runs in a separate `launch` block. If the API fails, a `Resource.Error` is sent but the flow stays alive — cached data continues to flow through the Room observer. For details specifically, when navigating from search, the venue is already cached so Room emits instantly while the API fetches full details in the background.
